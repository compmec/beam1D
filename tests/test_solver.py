...
def test_cantilever2pts():
    ...
    Korig = [
        [
            ...
            [[0, 0, 0, 32479.2348, 0, 0], [0, 0, 0, -32479.2348, 0, 0]],
            [
    ...
    Forig = [[0, 0, 0, 0, 0, 0], [0, -10, 0, 0, 0, 0]]
    Uorig = [[0, 0, None, None, None, 0], [None, None, None, None, None, None]]
    Korig = np.array(Korig)
    Forig = np.array(Forig)
    Uorig = np.array(Uorig)
    Ugood = [[0, 0, 0, 0, 0, 0], [0, -78.9459043, 0, 0, 0, -0.118418856]]
    Fgood = [[0, 10, 0, 0, 0, 10000], [0, -10, 0, 0, 0, 0]]
...
@pytest.mark.order(1)
@pytest.mark.dependency(depends=["test_begin", "test_cantilever2pts"])
def test_random_notsingular_matrix():
    ...
    for i in range(ntests):
        ...
        nunk = np.random.randint(
            int(0.1 * ndofs * npts), int(0.9 * ndofs * npts)
        )  # Number of values to set as unknown
        pairsunk = []
        counter = 0
        while counter < nunk:
...
@pytest.mark.order(1)
@pytest.mark.dependency(
    depends=["test_begin", "test_cantilever2pts", "test_random_notsingular_matrix"]
)
def test_end():
    pass
