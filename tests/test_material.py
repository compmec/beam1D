...
def test_fails():
    ...
    with pytest.raises(ValueError):
        Isotropic(K=100e3)
    with pytest.raises(ValueError):
        Isotropic(E=210e3, nu=0.495)  # Incompressible
    with pytest.raises(ValueError):
        Isotropic(E=210e3, nu=-0.2)
    with pytest.raises(ValueError):
        Isotropic(E=210e3, nu=0.7)
    with pytest.raises(ValueError):
        Isotropic(Ef=210e3, nu=0.3)
    with pytest.raises(ValueError):
        Isotropic(mu=70e3, nu=0.3)
    with pytest.raises(ValueError):
        Isotropic(E=210e3, G=60e3, nu=0.4)
    with pytest.raises(ValueError):
        Isotropic(E=-210e3, G=60e3)
    with pytest.raises(TypeError):
        Isotropic(E="210", G=60e3)
